...
class SpConvBlock(nn.Module):
    ...
    def sp_forward(self, input):
        ...
        if self.activation is None:
            ...
            act_sec_ord_grad = dd_swish(bn_out) # batch_size * n_out * h * w
        else:
        ...
        if self.activation == 'swish':
            out = swish(bn_out) + aux
        else:
            out = self.softplus(bn_out) + aux
        return out
...
if __name__ == '__main__':
    ...
    sp_conv = SpConvBlock(3, 32, kernel_size=(2, 3), stride=(3,1), padding=(9,0))
