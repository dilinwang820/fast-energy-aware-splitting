...
def main():
    ...
    args.model_save_path = os.path.join(args.save, os.path.basename(args.load))

    #prefix = os.path.basename(args.load).replace(".pth.tar", "")
    #if args.retrain:
    #    prefix = "{}_retrain".format(prefix)
    #else:
    #    prefix = "{}_finetune".format(prefix)
    #args.model_save_path = \
    #        os.path.join(args.save, '{}.pth.tar'.format(prefix))
    ...
    if not args.sp:
        from mobilenetv1 import mobilenetv1, Block
        model = mobilenetv1(cfg= checkpoint['cfg'])
    else:
        from sp_mbnet import sp_mbnet
        model = sp_mbnet(cfg= checkpoint['cfg'])
    cfg = model.cfg
