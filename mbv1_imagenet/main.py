...
parser.add_argument('--sr', action='store_true',
                    help='use sparsity regularization')
parser.add_argument('--sp', action='store_true',
                    help='use splitting configuration')
parser.add_argument('--sp_cfg', type=str, default='0.5',
                    help='splitting base start point')
...
if args.sp:
    ...
    from mobilenetv1 import Block, ConvBlock
...
def updateBN(model, sparsity):
    for m in model.modules():
        if isinstance(m, Block):
            m.bn2.weight.grad.data.add_(sparsity*torch.sign(m.bn2.weight.data))  # L1 
