...
def print_model_param_nums(model=None):
    ...
    if hasattr(model, 'net_params'):
        total = sum([param.nelement() if param.requires_grad else 0 for param in model.net_params])
    else:
...
def print_model_param_flops(model=None, input_res=224, multiply_adds=True):
    ...
    def foo(net):
        ...
        if not childrens:
            ...
            if isinstance(net, torch.nn.ReLU) or isinstance(net, Swish):
